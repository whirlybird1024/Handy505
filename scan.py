...
tf.keras.layers.Conv2D(2,3,use_bias=False)

# define the configuration dictionary
CONFIG = {
    # define the paths to the CNN
    "model_path": os.path.sep.join(["data", "pokedex.model"]),
...
class scanner():
    ...
    def scanning(self):
        ...
        mon = pygame.image.load("pics/sugimori/48.png")
        mon = pygame.transform.scale(mon, (350, 350))
        self.screen.blit(mon, (50, 50))
        ...
        if clicked and count < CONFIG["display_for"]:
            ...
            frame = imutils.resize(
                frame, width=480, inter=cv2.INTER_NEAREST)
            ...
            clicked = False
            predLabel = None
            pyFrameImg = pygame.surfarray.make_surface(frame)
            #print("should be blitting to screen")
            screen.blit(pyFrameImg, (0, 0))
...
def classify(image):
    ...
    proba = model.predict(image, verbose=1)[0]
